...
async function getClientBalance(prisma, clientId) {
  ...
  const [purchasesAgg, paymentsAgg] = await Promise.all([
    prisma.invoice.aggregate({
      where: { clientId: cId, type: 'purchase', isHistoricalCopy: false },
      _sum: { amount: true }
    }),
    prisma.invoice.aggregate({
      where: { clientId: cId, type: 'payment', isHistoricalCopy: false },
      _sum: { amount: true }
    })
  ]);
...
async function getAllClientBalances(prisma) {
  const [purchaseGroups, paymentGroups] = await Promise.all([
    prisma.invoice.groupBy({
      by: ['clientId'],
      where: { type: 'purchase', isHistoricalCopy: false },
      _sum: { amount: true }
    }),
    prisma.invoice.groupBy({
      by: ['clientId'],
      where: { type: 'payment', isHistoricalCopy: false },
      _sum: { amount: true }
    })
  ]);
