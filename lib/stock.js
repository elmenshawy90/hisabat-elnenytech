...
/**
 * Reverse a previously deducted stock quantity for an item in base unit and record an 'adjustment' StockLog.
 * @param {import('@prisma/client').PrismaClient} dbClient
 * @param {number} itemId
 * @param {number} quantityBase - Positive number to add back to stock
 * @param {number|null} [invoiceId=null]
 * @param {string} [notes=""]
 * @returns {Promise<import('@prisma/client').StockLog>}
 */
async function reverseStock(dbClient, itemId, quantityBase, invoiceId = null, notes = '') {
  const iId = Number(itemId);
  if (isNaN(iId)) {
    throw new Error('معرف الصنف غير صالح');
  }

  const qtyToAdd = Math.abs(Number(quantityBase) || 0);

  const executeReverse = async (tx) => {
    const latestLog = await tx.stockLog.findFirst({
      where: { itemId: iId },
      orderBy: [
        { id: 'desc' }
    ...
    const currentBalance = latestLog ? (Number(latestLog.balanceAfter) || 0) : 0;
    const balanceAfter = currentBalance + qtyToAdd;
  ...
  if (typeof dbClient.$transaction === 'function') {
    return await dbClient.$transaction(executeReverse);
  } else {
    return await executeReverse(dbClient);
  }
}
